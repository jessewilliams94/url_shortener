...
const ViewData = () => {
  ...
  const scanTable = async () => {
    try {
      data = await ddbDocClient.send(new ScanCommand({ TableName: "Urls" }));
          ...
          <div className="py-2 inline-block min-w-full sm:px-6 lg:px-8">
            <div className="overflow-hidden">
              <table className="min-w-full">
                ...
                <tbody>
                  {tableData.map((item) => (
                    <tr className="border-b" key={item.id + item.link_original}>
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                        {item.id}
                      </td>
                      <td className={Styles.tableData}>{item.link_original}</td>
                      <td className={Styles.tableData}>{item.lastName}</td>
                      <td className={Styles.tableData}>{item.link_shortened}</td>
